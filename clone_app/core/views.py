...
from core.models import FollowerModel,PostModel,LikeModel,CommentModel,SavedPost
...
class FollowDoneView(View):
    def post(self,request,*args,**kwargs):
        ...
        followed_user_obj = User.objects.get(pk=follow_user_id)
        try:
            FollowerModel.objects.get(user = request.user,followed = followed_user_obj)
        except Exception as e:
            FollowerModel.objects.create(followed = followed_user_obj)
...
class UnfollowDoneView(View):
    def post(self,request,*args,**kwargs):
        ...
        try:
            follow_obj = FollowerModel.objects.get(user = request.user,followed = unfollowed_user_obj)
            follow_obj.delete()
        except Exception as e:
            pass
        return redirect(request.META.get('HTTP_REFERER'))
...
class PostDetailView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        try:
            ...
            return redirect(request.META.get('HTTP_REFERER'))
        try:
            LikeModel.objects.get(user=request.user,post_id = post_id)
...
class PostLikeView(View):
    def post(self,request,*args,**kwargs):
        ...
        try:
            ...
            LikeModel.objects.create(post_id=post_id)
...
class PostUnlikeView(View):
    def post(self,request,*args,**kwargs):
        post_id = kwargs.get('id')
        try:
            like_obj = LikeModel.objects.get(user = request.user,post_id = post_id)
            like_obj.delete()
        except Exception as e:
            LikeModel.objects.create(post_id=post_id)
...
class PostCommentView(View):
    def post(self,request,*args,**kwargs):
        ...
        CommentModel.objects.create(post_id=post_id,text = comment_text)
...
class PostSaveView(View):
    def post(self,request,*args,**kwargs):
        ...
        try:
            post_obj = PostModel.objects.get(post_id = post_id)
        except Exception as e:
            pass
...
class ExplorePostsView(View):
    ...
    def get(self,request,*args,**kwargs):
        all_posts = PostModel.objects.annotate(count = Count('likemodel')).order_by('-count')
